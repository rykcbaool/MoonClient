package dev.ryk.mod.gui.clickgui.components.impl;

import dev.ryk.MoonClient;
import dev.ryk.api.utils.math.Animation;
import dev.ryk.mod.modules.impl.client.ClickGui;
...
public class EnumComponent extends Component {
	...
	public boolean draw(int offset, DrawContext drawContext, float partialTicks, Color color, boolean back) {
		...
		if (ClickGui.INSTANCE.mainEnd.booleanValue) {
			Render2DUtil.drawRectHorizontal(matrixStack, (float) x + 1, (float) y + 1, (float) width - 2, (float) defaultHeight - (ClickGui.INSTANCE.maxFill.getValue() ? 0 : 1), hover ? ClickGui.INSTANCE.mainHover.getValue() : MoonClient.GUI.getColor(), ClickGui.INSTANCE.mainEnd.getValue());
		} else {
			Render2DUtil.drawRect(matrixStack, (float) x + 1, (float) y + 1, (float) width - 2, (float) defaultHeight - (ClickGui.INSTANCE.maxFill.getValue() ? 0 : 1), hover ? ClickGui.INSTANCE.mainHover.getValue() : MoonClient.GUI.getColor());
		}
