...
public class AutoPush extends Module {
    ...
    public static void pistonFacing(Direction i) {
        if (i == Direction.EAST) {
            MoonClient.ROTATION.snapAt(-90.0f, 5.0f);
        } else if (i == Direction.WEST) {
            MoonClient.ROTATION.snapAt(90.0f, 5.0f);
        } else if (i == Direction.NORTH) {
            MoonClient.ROTATION.snapAt(180.0f, 5.0f);
        } else if (i == Direction.SOUTH) {
            MoonClient.ROTATION.snapAt(0.0f, 5.0f);
        }
    }
            ...
            if (BlockUtil.clientCanPlace(piston)) {
                ...
                if (BlockUtil.getPlaceSide(piston, placeRange.getValue()) != null) {
                    ...
                    if (canPower) {
                        ...
                        if (side != null) {
                            if (rotate.getValue()) MoonClient.ROTATION.lookAt(piston.offset(side), side.getOpposite());
                            ...
                            if (rotate.getValue() && yawDeceive.getValue()) MoonClient.ROTATION.lookAt(piston.offset(side), side.getOpposite());
...
/*                        if (mine.getValue()) {
    ...
    private boolean isTrueFacing(BlockPos pos, Direction facing) {
        ...
        float[] rotation = MoonClient.ROTATION.getRotation(directionVec);
    ...
    private Boolean canPush(PlayerEntity player) {
        ...
        if (!allowWeb.getValue() && MoonClient.PLAYER.isInWeb(player)) return false;
        float[] offset = new float[]{-0.25f, 0f, 0.25f};
        ...
        return progress > surroundCheck.getValue() - 1 || MoonClient.HOLE.isHard(new BlockPosX(player.getX(), player.getY() + 0.5, player.getZ()));
    }
