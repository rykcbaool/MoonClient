...
import dev.ryk.api.utils.world.BlockPosX;
import dev.ryk.api.utils.world.BlockUtil;
import dev.ryk.asm.accessors.IEntity;
...
import dev.ryk.mod.modules.impl.exploit.Blink;
import dev.ryk.mod.modules.impl.player.PacketMine;
import dev.ryk.mod.modules.impl.render.ExplosionSpawn;
...
import static dev.ryk.api.utils.world.BlockUtil.getBlock;
import static dev.ryk.api.utils.world.BlockUtil.hasCrystal;
...
public class AutoCrystal extends Module {
    ...
    public AutoCrystal() {
        ...
        setChinese("自动水晶");
        INSTANCE = this;
        MoonClient.EVENT_BUS.subscribe(new CrystalRender());
    }
    ...
    private void doBreak(BlockPos pos) {
        ...
        for (EndCrystalEntity entity : BlockUtil.getEndCrystals(new Box(pos.getX(), pos.getY(), pos.getZ(), pos.getX() + 1, pos.getY() + 2, pos.getZ() + 1))) {
            ...
            if (crystalPos != null && displayTarget != null && lastDamage >= getDamage(displayTarget) && afterBreak.getValue()) {
                if (!yawStep.getValue() || !checkFov.getValue() || MoonClient.ROTATION.inFov(entity.getPos(), fov.getValueFloat())) {
                    doPlace(crystalPos);
                }
            }
    ...
    private boolean faceVector(Vec3d directionVec) {
        if (!yawStep.getValue()) {
            ...
            if (MoonClient.ROTATION.inFov(directionVec, fov.getValueFloat())) {
                return true;
            }
        }
