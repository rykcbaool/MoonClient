...
public class TimerModule extends Module {
	...
	public void onRender2D(DrawContext drawContext, float tickDelta) {
		...
		if (MovementUtil.isMoving() && !MoonClient.PLAYER.insideBlock) {
			...
			if (timer2.passed(lastMs)) {
				...
				if (smooth.getValue()) {
					double timer = MoonClient.TIMER.getDefault() + (1 - end.ease(ease.getValue())) * (shiftTimer.getValueFloat() - 1) * (lastMs / accumulate.getValue());
					MoonClient.TIMER.set((float) Math.max(MoonClient.TIMER.getDefault(), timer));
				} else {
					MoonClient.TIMER.set(shiftTimer.getValueFloat());
				}
			}
		} else {
			if (moving) {
