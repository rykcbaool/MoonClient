...
import dev.ryk.api.utils.math.AnimateUtil;
...
import static dev.ryk.api.utils.world.BlockUtil.*;
...
public class AutoAnchor extends Module {
	...
	public AutoAnchor() {
		...
		setChinese("自动重生锚");
		INSTANCE = this;
		MoonClient.EVENT_BUS.subscribe(new AnchorRender());
	}
	...
	public void onUpdate() {
		...
		if (currentPos != null) {
			...
			boolean shouldSpam = this.spam.getValue() && (!mineSpam.getValue() || MoonClient.BREAK.isMining(currentPos, false));
			if (shouldSpam) {
	...
	public BlockPos getHelper(BlockPos pos) {
		for (Direction i : Direction.values()) {
			if (checkMine.getValue() && MoonClient.BREAK.isMining(pos.offset(i))) continue;
