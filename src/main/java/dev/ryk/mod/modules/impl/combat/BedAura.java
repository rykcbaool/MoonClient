...
public class BedAura extends Module {
	...
	public void onEnable() {
		lastYaw = MoonClient.ROTATION.lastYaw;
		lastPitch = MoonClient.ROTATION.lastPitch;
	}
	...
	public void onRotate(RotateEvent event) {
		if (!rotate.getValue() && yawDeceive.getValue()) {
			...
			float[] newAngle = injectStep(MoonClient.ROTATION.getRotation(directionVec), steps.getValueFloat());
			lastYaw = newAngle[0];
	...
	private void doPlace(BlockPos pos) {
		...
		for (Direction i : Direction.values()) {
			...
			if ((legacy.getValue() && BlockUtil.canReplace(pos.offset(i)) || BlockUtil.clientCanPlace(pos.offset(i), false)) && BlockUtil.canClick(pos.offset(i).down()) && (!checkMine.getValue() || !MoonClient.BREAK.isMining(pos.offset(i)))) {
				facing = i;
				break;
			}
		}
	...
	private boolean canPlaceBed(BlockPos pos) {
		if (BlockUtil.canReplace(pos) && (!checkMine.getValue() || !MoonClient.BREAK.isMining(pos)) && (!legacy.getValue() || BlockUtil.canClick(pos.down()))) {
			for (Direction i : Direction.values()) {
				if (i == Direction.UP || i == Direction.DOWN) continue;
	...
	private boolean isTrueFacing(BlockPos pos, Direction facing) {
		...
		return Direction.fromRotation(MoonClient.ROTATION.getRotation(hitVec)[0]) == facing;
	}

	private boolean isTrueFacingNow(Direction facing) {
		return Direction.fromRotation(MoonClient.ROTATION.lastYaw) == facing;
	}
	public enum Page {
		General,
		Rotate,
	...
	public boolean faceVector(Vec3d directionVec) {
		if (!yawStep.getValue()) {
			MoonClient.ROTATION.lookAt(directionVec);
			return true;
		} else {
			this.directionVec = directionVec;
			if (MoonClient.ROTATION.inFov(directionVec, fov.getValueFloat())) return true;
		}
