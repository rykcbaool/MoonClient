...
import static dev.ryk.api.utils.world.BlockUtil.canReplace;
...
public class Burrow extends Module {
    ...
    public void onUpdate() {
        if (MoonClient.PLAYER.isInWeb(mc.player)) {
            webTimer.reset();
            return;
        }
        ...
        if (this.rotate.getValue() == RotateMode.Bypass) {
            MoonClient.ROTATION.snapAt(MoonClient.ROTATION.rotationYaw, 90);
        }
    ...
    private void gotoPos(BlockPos offPos) {
        ...
        if (rotate.getValue() == RotateMode.None) {
            ...
            mc.getNetworkHandler().sendPacket(new PlayerMoveC2SPacket.Full(offPos.getX() + 0.5, mc.player.getY() + 0.1, offPos.getZ() + 0.5, MoonClient.ROTATION.rotationYaw, 90, false));
        }
    }
    ...
    private boolean canPlace(BlockPos pos) {
        ...
        if (detectMine.getValue() && MoonClient.BREAK.isMining(pos)) {
            return false;
        }
