...
import dev.ryk.mod.modules.impl.movement.MoveFix;
...
public class RotationManager implements Wrapper {
    ...
    public void snapBack() {
        mc.getNetworkHandler().sendPacket(new PlayerMoveC2SPacket.Full(mc.player.getX(), mc.player.getY(), mc.player.getZ(), MoonClient.ROTATION.rotationYaw, MoonClient.ROTATION.rotationPitch, mc.player.isOnGround()));
    }
    ...
    public void snapAt(Vec3d directionVec) {
        ...
        if (AntiCheat.INSTANCE.noSpamRotation.getValue()) {
            if (MathHelper.angleBetween(angle[0], MoonClient.ROTATION.lastYaw) < AntiCheat.INSTANCE.fov.getValueFloat() && Math.abs(angle[1] - MoonClient.ROTATION.lastPitch) < AntiCheat.INSTANCE.fov.getValueFloat()) {
                return;
            }
        }
    ...
    public void update(MovementPacketsEvent event) {
        if (MoveFix.INSTANCE.isOn() && !BaritoneModule.isActive()) {
            ...
            MoonClient.EVENT_BUS.post(event1);
    ...
    private void updateNext() {
        ...
        MoonClient.EVENT_BUS.post(rotateEvent);
    ...
    public void onLastRotation(RotateEvent event) {
        ...
        MoonClient.EVENT_BUS.post(lookAtEvent);
